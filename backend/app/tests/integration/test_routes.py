...
def test_get_bookshelves_after_create():
    result = [{'id': 1, 'title': 'Bookshelf Title', 'description': 'Bookshelf Description'}]
    response = client.get("/bookshelves")
    assert response.status_code == 200
...
def test_get_bookshelf():
    bookshelf_id = 1
    result = {'id': bookshelf_id, 'title': 'Bookshelf Title', 'description': 'Bookshelf Description'}
...
def test_get_bookshelf_after_patch():
    bookshelf_id = 1
    result = {'id': bookshelf_id, 'title': 'New Bookshelf Title', 'description': 'New Bookshelf Description'}
    response = client.get(f"/bookshelves/{bookshelf_id}")
    assert response.status_code == 200
    assert response.json() == result
